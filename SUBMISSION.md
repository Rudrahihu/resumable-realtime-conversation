...
Runs directly against the orchestrator (no live sockets, no real model —
fully deterministic). It generates 35 ordered events for one run
...
**Failure/recovery scenario shown in the demo video:** [describe exactly
what you recorded here — e.g. "I killed the backend process mid-stream
with Ctrl+C, showed the 'Reconnecting' state and disabled composer,
restarted the backend, and showed the reply resume with no duplicated or
missing text, plus the restart-reconciliation log line on the server."]
...
- **SSE instead of Socket.io** — would have worked for the one-directional
  ...
  already had production experience with it.
...
<!-- Adjust/expand this section to accurately describe your own process. -->

## Credibility note

<!--
Fill this in yourself — this needs to be your own real experience, not
something I can write for you. The template asks for:
